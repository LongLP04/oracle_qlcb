    ...
    if st.session_state.ticket_query:
        ...
        if success and data:
            ...
            for row in data:
                ...
                cols[0].write(f"Vé {mave} | {row.get('DIEMDI')} → {row.get('DIEMDEN')} | Ghế {row.get('SOGHE')}")
                    ...
                    try:
                        ...
                        if success_time and time_rows:
                            ...
                            if ngay_bay and db_now and giave:
                                ...
                                refund_amount = int(giave * _refund_rate(days_to_depart))
                                fee_amount = int(giave - refund_amount)
                ...
                if preview:
                    st.warning(
                        "Xác nhận hủy vé? Hoàn: {:,.0f}đ | Phí: {:,.0f}đ | Đặt trước {:.1f} ngày".format(
                            preview["refund"], preview["fee"], preview["days"]
                        )
                    )
